...
namespace mythril {
	...
	CompiledBarrier CompileReadOperation(CTX& ctx, const ReadSpec& readSpec) {
		...
		const VkImageMemoryBarrier2 barrier = vkinfo::CreateImageMemoryBarrier2(
				...
				false);
		return {barrier, readSpec.texture, true};
	}
	CompiledBarrier CompileWriteOperation(CTX& ctx, const WriteSpec& writeSpec, VkImageLayout vkNewImageLayout) {
		...
		return {barrier, writeSpec.texture, false};
	}

	// we make this call frequently, so lets alias it
#define UPDATE_CURRENT_LAYOUT(handle, layout) ctx._texturePool.get(handle)->_vkCurrentImageLayout = layout;
	...
	void RenderGraph::compile(CTX& ctx) {
		...
		this->_initialLayoutsPerFrame.clear();

		// this MUST be done outside and BEFORE the building of compiled passes,
		// as we make updates to the vkCurrentImageLayout of those images
		// and therefore our initial layouts would be incorrect
		for (const PassSource& source: this->_sourcePasses) {
			// iterate through rendergraph and store inital layouts to resume at beginning of execute
			for (const ReadSpec& read : source.readOperations) {
				if (!_initialLayoutsPerFrame.contains(read.texture)) {
					_initialLayoutsPerFrame[read.texture] = ctx.viewTexture(read.texture).getImageLayout();
				}
			}
			for (const WriteSpec& write : source.writeOperations) {
				if (!_initialLayoutsPerFrame.contains(write.texture)) {
					_initialLayoutsPerFrame[write.texture] = ctx.viewTexture(write.texture).getImageLayout();
				}
			}
		}
		...
		for (const PassSource& source: this->_sourcePasses) {
			...
			for (const ReadSpec& readOperation : source.readOperations) {
				if (ctx.viewTexture(readOperation.texture).getImageLayout() != readOperation.expectedLayout) {
				...
				UPDATE_CURRENT_LAYOUT(readOperation.texture, readOperation.expectedLayout);
			}
				...
				for (const WriteSpec& writeOperation: source.writeOperations) {
					...
					if (currentTexture.isDepthAttachment()) {
						...
						compiled.depthAttachment = DepthAttachmentInfo{
								...
								.imageLayout = currentTexture.getImageLayout(),
						...
						constexpr VkImageLayout vk_new_image_layout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
						if (currentTexture.getImageLayout() != vk_new_image_layout) {
							VkImageMemoryBarrier2 barrier = vkinfo::CreateImageMemoryBarrier2(
						...
						UPDATE_CURRENT_LAYOUT(writeOperation.texture, vk_new_image_layout);
					} else {
						...
						if (writeOperation.resolveTexture.has_value()) {
							...
							if (resolveTexture.getImageLayout() != VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL) {
								...
								VkImageMemoryBarrier2 resolveBarrier = vkinfo::CreateImageMemoryBarrier2(
									...
									resolveTexture.getImageLayout(),
									vk_new_image_layout,
									true);
								compiled.preBarriers.push_back({resolveBarrier, writeOperation.resolveTexture.value()});
							}
							UPDATE_CURRENT_LAYOUT(writeOperation.resolveTexture.value(), vk_new_image_layout);
						}
	...
	static void ValidateReadImageLayouts(const CTX& ctx, const std::vector<PassSource>& passes) {
		for (const PassSource& pass : passes) {
			for (const ReadSpec& read : pass.readOperations) {
				...
				ASSERT_MSG(read.expectedLayout == currentImageLayout, "Pass '{}': Texture '{}' does not have expected layout '{}' instead is in '{}'!",
					pass.name,
	...
	void RenderGraph::execute(CommandBuffer& cmd) {
		...
		// reset the images image layout to their initial upon entering the RenderGraph,
		// this ensures that the ImageMemoryBarriers are correct
		for (const PassCompiled& pass : _compiledPasses) {
			cmd._activePass = pass;
			// texture transitions before callback
			if (!pass.preBarriers.empty()) {
				std::vector<VkImageMemoryBarrier2> barriers;
				barriers.reserve(pass.preBarriers.size());
				for (const CompiledBarrier& cb : pass.preBarriers) {
					VkImageMemoryBarrier2 barrier = cb.barrier;
					auto& tex = cmd._ctx->viewTexture(cb.textureHandle);
					barrier.oldLayout = tex.getImageLayout();
					if (barrier.oldLayout == barrier.newLayout) {
						continue;
					}
					ASSERT(barrier.oldLayout != barrier.newLayout);
					// LOG_CUSTOM("Inserting barrier for texture '{}' from '{}' to '{}'",
					// 	tex.getDebugName(),
				...
				LOG_CUSTOM("Submitting {} barriers", barriers.size());
				ASSERT(cmd._wrapper->_cmdBuf != VK_NULL_HANDLE);
			...
			// location matters, this must be here
#ifdef DEBUG
